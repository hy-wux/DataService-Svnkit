...
spring.application.name=dataservice-svnkit-producer-subversionhttp
...
软件运行后，需要进行服务器配置才能正常管理服务器。在添加界面，微服务名称中，填写前文配置的生产者微服务名称，即：`dataservice-svnkit-producer-subversionhttp`。
