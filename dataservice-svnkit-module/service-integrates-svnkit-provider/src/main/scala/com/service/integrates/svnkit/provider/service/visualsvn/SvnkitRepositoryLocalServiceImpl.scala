package com.service.integrates.svnkit.provider.service.visualsvn
...
@Service
@ConditionalOnProperty(value = Array("service.svnkit.version"), havingValue = "visualsvn")
class SvnkitRepositoryLocalServiceImpl extends SvnkitRepositoryOperationService {
  ...
  override def createLocalRepository(path: File, enableRevisionProperties: Boolean, force: Boolean): SVNURL = {
    ...
    new File(path, property.getAuthzFile).createNewFile()
    url
  }
  ...
  override def listRights(repositoryName: String, repositoryPath: String): util.Map[String, util.Map[String, String]] = {
    ...
    val repositoryRoot = new File(property.getPath, repositoryName)

    val ini = iniService.loadConfig(new File(repositoryRoot, property.getAuthzFile))

    (0 until sectionPaths.length).foreach(i => {
      val sectionName = if (i == 0) "/" else (0 to i).map(sectionPaths(_)).mkString("/")
  ...
  override def addRights(repositoryName: String, repositoryPath: String, values: util.Map[String, String]): Unit = {
    val repositoryRoot = new File(property.getPath, repositoryName)
    iniService.addConfigs(new File(repositoryRoot, property.getAuthzFile), repositoryPath, values)
  }
  ...
  override def deleteRights(repositoryName: String, repositoryPath: String, names: Array[String]): Unit = {
    iniService.deleteConfigs(new File(new File(property.getPath, repositoryName), property.getAuthzFile), repositoryPath, names)
  }
  ...
  private def objectHaveRight(objects: List[String]): List[String] = {
    // 仓库根目录
    val repositoryNames = new File(property.getPath)
      // 根目录下的文件
      .listFiles()
      // 找到包含授权文件的
      .filter(repository => {
      new File(repository, property.getAuthzFile).exists()
    })
      .filter(repository => {
        val ini = iniService.loadConfig(new File(repository, property.getAuthzFile))
        ini.containsKey("/") && ini.get("/").keySet().filter(objects.contains(_)).size > 0
      }).map(_.getName).toList
